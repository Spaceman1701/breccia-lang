...
int bc_list_new(size_t element_size, size_t inital_capactiy, Bc_List *list) {
    ...
    if (inital_capactiy == 0) {
        inital_capactiy = 1; //on some implementations malloc of 0 size will return NULL
    }
...
int bc_list_expand_capacity(Bc_List *list, size_t new_capacity) {
    Bc_ListElement* new_data = (Bc_ListElement*) realloc(list->data, list->element_size*new_capacity);
    if (!new_data) {
        return -1;
    }
    list->data = new_data;
...
Bc_ListElement* bc_list_append_ptr(Bc_List *list) {
    if (list->capacity == list->length) {
        ...
        if (err) {
            return NULL; //properly you should be checking for null when using this...
        }
    }
